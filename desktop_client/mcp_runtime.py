...
class DesktopClientMCPRuntime:
    def __init__(self, config: DesktopClientConfig, manager: MCPManager | None = None):
        self._config = config
    ...
    def tool_definitions(self) -> list[dict[str, Any]]:
        return [dict(payload) for payload in self._tool_map.values()]

    def can_handle(self, tool_key: str) -> bool:
        return tool_key in self._tool_map

    async def call_tool(self, tool_key: str, arguments: dict[str, Any]) -> dict[str, Any]:
        entry = self._tool_map.get(tool_key)
        if entry is None:
        ...
        for server_name, tools in self._manager.mcp_tools.items():
            for tool in tools:
                ...
                mapping[tool_key] = {
                    ...
                    "tags": ["desktop", "mcp", slug_tool_segment(server_name)],
