...
from desktop_client.policy import (
    DesktopClientPolicyError,
...
def build_file_read_handler(config: DesktopClientConfig) -> Handler:
    async def handler(arguments: dict[str, Any]) -> dict[str, Any]:
        raw_path = str(arguments.get("path") or "").strip()
        if not raw_path:
            raise DesktopClientPolicyError("path_required", "path is required")
...
def build_file_write_handler(config: DesktopClientConfig) -> Handler:
    async def handler(arguments: dict[str, Any]) -> dict[str, Any]:
        raw_path = str(arguments.get("path") or "").strip()
        if not raw_path:
            raise DesktopClientPolicyError("path_required", "path is required")
        ...
        if mode == "append":
            ...
            if target.exists():
                raise DesktopClientPolicyError("already_exists", f"File already exists: {target}")
...
def build_shell_exec_handler(config: DesktopClientConfig) -> Handler:
    policy = load_cmd_policy(config)

    async def handler(arguments: dict[str, Any]) -> dict[str, Any]:
        command = str(arguments.get("command") or arguments.get("cmd") or "").strip()
        if not command:
            raise DesktopClientPolicyError("command_required", "command is required")
        safety, reason = assess_command_safety(command, policy=policy)
        if safety != "safe":
            raise DesktopClientPolicyError("command_blocked", reason or "command blocked by policy")
        ...
        try:
            ...
            raise DesktopClientPolicyError("command_timeout", f"command timed out after {config.command_timeout_seconds} seconds") from exc

        stdout_text = stdout.decode("utf-8", errors="replace").strip()
        stderr_text = stderr.decode("utf-8", errors="replace").strip()
        if process.returncode != 0:
            raise DesktopClientPolicyError(
                "command_failed",
...
def build_workspace_analyze_handler(config: DesktopClientConfig) -> Handler:
    async def handler(arguments: dict[str, Any]) -> dict[str, Any]:
        raw_path = str(arguments.get("path") or "").strip()
        if not raw_path:
            raise DesktopClientPolicyError("path_required", "path is required")
...
def build_tool_handlers(config: DesktopClientConfig) -> dict[str, Handler]:
    return {
        "utility.echo": echo_handler,
        "workspace.analyze": build_workspace_analyze_handler(config),
        "file.read": build_file_read_handler(config),
        "file.write": build_file_write_handler(config),
        "shell.exec": build_shell_exec_handler(config),
    }
