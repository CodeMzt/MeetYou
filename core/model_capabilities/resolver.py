...
class ModelCapabilityResolver:
    def __init__(
        ...
        *,
        ...
        self._registry_path = (
            Path(registry_path)
            if registry_path
            else Path(__file__).with_name("default_registry.json")
        )
    ...
    def _cache_key(self, provider: str, model: str) -> str:
        provider_norm = self._normalize_provider(provider, model)
        return f"{provider_norm}::{model.strip().lower()}"
    ...
    def _normalize_provider(provider: str, model: str) -> str:
        ...
        if model_norm.startswith("deepseek-") and provider_norm in {"", "openai"}:
            return "deepseek"
    ...
    def _lookup_registry(self, provider: str, model: str) -> ModelCapability | None:
        ...
        for item in self._registry.get("entries", []):
            ...
            return ModelCapability(
                ...
                requires_manual_confirmation=bool(item.get("requires_manual_confirmation", False)),
            )
        return None
    ...
    def resolve(self, provider: str, model: str) -> ModelCapability:
        ...
        if isinstance(cache_entry, dict) and self._cache_entry_is_fresh(cache_entry):
            ...
            if capability.get("context_window"):
                return ModelCapability(
                    ...
                    requires_manual_confirmation=bool(capability.get("requires_manual_confirmation", False)),
                )
        ...
        diagnostic = f"Unknown model capability for provider={provider_norm} model={model}; using default fallback context={_DEFAULT_CONTEXT_LIMIT}."
