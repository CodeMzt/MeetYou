...
from tools.memory import recall_deep_memory
from tools.memory import save_deep_memory
...
async def input_brain(input_info,api_key,api_url,model):
    ...
    payload = {
        ...
        "tools": __tool_schemas_list["common_tools"]+__tool_schemas_list["memory_tools"],
        "stream": True
    }
    ...
    if __is_tool_call:
        ...
        elif __func_name == 'save_deep_memory':
            memory_text = __func_args_dict.get('memory_text')
            if not memory_text:
                result =  (f"Error: save_deep_memory function missing 'memory_text' argument\r\n")
            else:
                result = await save_deep_memory(memory_text)
        elif __func_name == 'recall_deep_memory':
            query_text = __func_args_dict.get('query_text')
            if not query_text:
                result =  (f"Error: recall_deep_memory function missing 'query_text' argument\r\n")
            else:
                result = await recall_deep_memory(query_text)
