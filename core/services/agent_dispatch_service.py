...
class AgentDispatchService:
        ...
        *,
        ...
        if capability is None:
            raise AgentDispatchError(
                ...
                f"Capability is unavailable on Desktop Agent: {selection.capability_id}",
        ...
        if session_row is None or thread_row is None or workspace is None:
            raise AgentDispatchError(
                ...
                "Desktop capability dispatch requires a valid session context",
        ...
        operation = self._operation_service.create_operation(
            ...
            title=title or f"Desktop capability: {capability_suffix}",
        ...
        if not dispatched:
            ...
            raise AgentDispatchError("agent_offline", f"Desktop Agent is offline: {selection.agent.agent_id}")
        ...
        try:
            ...
            raise AgentDispatchError("agent_timeout", f"Desktop Agent call timed out after {timeout_seconds} seconds", retryable=True) from exc
        finally:
            async with self._lock:
                self._pending.pop(call.call_id, None)
        ...
        if future is not None and not future.done():
            future.set_exception(
                AgentDispatchError(
                    ...
                    str((error or {}).get("message") or "Desktop Agent call failed"),
    ...
    def _select_target_agent(self, *, capability_suffix: str, session_id: str = "") -> DispatchSelection:
        ...
        for agent in self._agent_service.list_agents():
            if agent.agent_type != "desktop" or agent.status != "online":
                continue
        ...
        candidates.sort(
            key=lambda agent: (
                0 if getattr(agent, "owner_client_id", None) == getattr(session_row, "client_id", None) else 1,
                agent.agent_id,
            )
        )
        if candidates:
        ...
        raise AgentDispatchError(
            ...
            f"No online Desktop Agent is available for workspace: {workspace.workspace_id}",
