...
class Operation(TimestampMixin, Base):
    ...
    requested_by_actor_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("actors.id"), nullable=True)
    requested_by_run_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("runs.id"), nullable=True)
    ...
    execution_target_type: Mapped[str] = mapped_column(String(64), nullable=False, default="endpoint")
    execution_target_id: Mapped[str] = mapped_column(String(255), nullable=False, default="")
    target_endpoint_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("endpoints.id"), nullable=True)
...
class OperationCall(TimestampMixin, Base):
    ...
    capability_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("capabilities.id"), nullable=True)
    endpoint_capability_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("endpoint_capabilities.id"), nullable=True)
    target_endpoint_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("endpoints.id"), nullable=True)
    execution_target_id: Mapped[str] = mapped_column(String(255), nullable=False, default="")
    status: Mapped[str] = mapped_column(String(32), nullable=False, default="queued")
