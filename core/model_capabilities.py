"""Model capability resolution and per-request context budgeting."""
...
class ModelCapabilityResolver:
    def resolve(
        ...
        *,
        ...
        lp = dict(length_policy or {})
        thinking_cfg = dict((model_options or {}).get("thinking") or {})

        max_output = int(lp.get("reserved_response_tokens", 0) or max(768, int(context_window * 0.2)))
        reasoning_budget = 0
        thinking_enabled = thinking_cfg.get("enabled")
        if thinking_enabled is None or bool(thinking_enabled):
            reasoning_budget = int(thinking_cfg.get("budget_tokens", 0) or max(128, int(context_window * 0.04)))
        tool_result_budget = max(128, int(context_window * 0.08))

        # Clamp so input budget is never negative.
