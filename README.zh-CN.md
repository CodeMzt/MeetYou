...
- 提供 Electron + React 桌面 UI，覆盖 thread、operation、attachment、memory、workspace 和 settings。
...
| `POST` | `/runtime/attachments/upload-ticket` | 创建 attachment upload ticket。 |
| `PUT` | `/runtime/attachments/upload/{ticket_id}` | 上传 attachment bytes。 |
| `POST` | `/runtime/attachments/{attachment_id}/complete` | 上传后补全 attachment metadata。 |
| `GET` | `/runtime/attachments/{attachment_id}/download-ticket` | 创建 download ticket。 |
