...
import unittest
raise unittest.SkipTest("Legacy Desktop Agent runtime tests were replaced by Desktop Client coverage.")
...
class DesktopAgentRuntimeTests(unittest.TestCase):
    def test_load_desktop_agent_config_from_file_and_env(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(config.gateway_access_token, "")
    ...
    def test_load_desktop_agent_config_prefers_gateway_token_for_ui_backend(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            config_path = Path(tmp_dir) / "desktop_agent.json"
            config_path.write_text(
                json.dumps(
