...
- Research panel web-search controls must write `source_policy.web_search` and optional `source_policy.web_queries` into Core ResearchTask creation. If web search is disabled and no seed URL UI is provided, omit the `web` adapter instead of creating a guaranteed `WebSeedUrlsRequired` task.
