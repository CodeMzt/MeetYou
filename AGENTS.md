...
- WeChat's main supported path is the native ClawBot iLink Endpoint Provider (`endpoint_providers/clawbot.py`). Treat MeetWeChat/old WeChatBot as legacy failed support only; do not add new compatibility shims that force the ClawBot iLink flow into the old `/v1/events` or ACK shape.
