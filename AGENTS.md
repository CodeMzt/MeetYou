...
- V5 research adapter configuration lives in environment variables: `MEETYOU_RESEARCH_ADAPTER_BASE_URL`, `MEETYOU_RESEARCH_ADAPTER_TOKEN`, `MEETYOU_RESEARCH_PROVIDER`, `MEETYOU_RESEARCH_TIMEOUT_SECONDS`, and `MEETYOU_RESEARCH_ADAPTER_REQUIRED`. `MEETYOU_RESEARCH_TIMEOUT_SECONDS` is a per-request adapter HTTP guard, not a total research run deadline. Adapter service process settings use `MEETYOU_RESEARCH_ADAPTER_HOST`, `MEETYOU_RESEARCH_ADAPTER_PORT`, and provider-specific keys such as `OPENAI_API_KEY` / `TAVILY_API_KEY`.
...
- The research adapter may bridge Core `MEETYOU_API_KEY` to in-process `OPENAI_API_KEY` when the external provider SDK requires the OpenAI variable name; do not log or persist the bridged secret.
