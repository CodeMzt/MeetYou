...
- WeChat's main supported path is the official ClawBot/OpenClaw iLink Endpoint Provider (`endpoint_providers/clawbot.py`). Treat MeetWeChat/old WeChatBot as legacy failed support only; do not add new compatibility shims that force the official ClawBot flow into the old `/v1/events` or ACK shape.
- ClawBot WeChat must remain an Endpoint Provider. It may store OpenClaw account cursors/context tokens locally, but Core still owns Thread, Message, Run, Delivery, EndpointAddress truth, and final assistant Message persistence.
