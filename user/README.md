...
- `agent_access_token`: Agent 专用访问令牌兼容兜底；`/agent/ws` 主路径优先读 `MEETYOU_AGENT_WS_ACCESS_TOKEN`，其次 `MEETYOU_AGENT_ACCESS_TOKEN`，不会回退到 Gateway token
...
- `core_base_url`: Core Service 基地址；runtime 会把它转换为正式入口 `WSS /agent/ws`
- `agent_access_token`: Agent 专用访问令牌兼容兜底；`edge-agent` 优先读 `MEETYOU_EDGE_ACCESS_TOKEN`，其次 `MEETYOU_AGENT_WS_ACCESS_TOKEN` / `MEETYOU_AGENT_ACCESS_TOKEN`，不会回退到 Gateway token
...
- Agent 鉴权优先读各自专用 env，再回退到共享 `MEETYOU_AGENT_WS_ACCESS_TOKEN` / `MEETYOU_AGENT_ACCESS_TOKEN`；不会把 `MEETYOU_GATEWAY_ACCESS_TOKEN` 当作 `/agent/ws` 的 Agent token
