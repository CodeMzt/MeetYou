...
class DesktopAgentPolicyError(RuntimeError):
...
def ensure_readable_path(config: DesktopAgentConfig, raw_path: str) -> Path:
    ...
    if not is_path_allowed(target, config.resolved_read_roots):
        raise DesktopAgentPolicyError("path_not_allowed", f"Requested path is outside allowed read roots: {target}")
    if not target.exists() or not target.is_file():
        raise DesktopAgentPolicyError("file_not_found", f"Requested file does not exist: {target}")
    return target


def ensure_workspace_path(config: DesktopAgentConfig, raw_path: str) -> Path:
    target = (config.workspace_root / raw_path).resolve() if not Path(raw_path).is_absolute() else Path(raw_path).resolve()
    if not is_path_allowed(target, config.resolved_read_roots):
        raise DesktopAgentPolicyError("path_not_allowed", f"Requested path is outside allowed read roots: {target}")
    if not target.exists() or not target.is_dir():
        raise DesktopAgentPolicyError("directory_not_found", f"Requested directory does not exist: {target}")
    return target


def ensure_writable_path(config: DesktopAgentConfig, raw_path: str) -> Path:
    target = (config.workspace_root / raw_path).resolve() if not Path(raw_path).is_absolute() else Path(raw_path).resolve()
    if not is_path_allowed(target, config.resolved_write_roots):
        raise DesktopAgentPolicyError("path_not_allowed", f"Requested path is outside trusted write roots: {target}")
    return target


def load_cmd_policy(config: DesktopAgentConfig) -> dict:
