...
def load_edge_agent_config(config_file_path: str | None = None) -> EdgeAgentConfig:
    ...
    return EdgeAgentConfig(
        core_base_url=str(os.environ.get("MEETYOU_EDGE_BASE_URL") or payload.get("core_base_url") or "http://127.0.0.1:8000").strip(),
        mqtt_broker_url=str(os.environ.get("MEETYOU_EDGE_MQTT_URL") or payload.get("mqtt_broker_url") or "mqtt://127.0.0.1:1883").strip(),
        agent_access_token=str(
        ...
        pull_interval_seconds=max(int(payload.get("pull_interval_seconds") or 10), 1),
