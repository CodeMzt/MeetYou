...
- WeChat iLink Bot now uses inbound and outbound queues. Inbound handling keeps per-user order while allowing cross-user workers; outbound `sendmessage` is queued from `/client/ws` callbacks and sent with conservative rate limiting.
...
- `.venv\Scripts\python.exe -m unittest tests.test_web_search tests.test_wechat_ilink_adapter tests.test_wechat_ilink_client tests.test_tool_parallel_execution`
