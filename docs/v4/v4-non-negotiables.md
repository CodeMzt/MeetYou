...
13. V4 HTTP facade 是 `/runtime/*`；桌面本地 `/desktop/*` 只能代理到 `/runtime/*`、`/operator/*`、`/developer/*`，不得代理旧 `/client/*`。
14. 不保留 `/client/ws`、`source_client_id`、`target_client_id`、`ClientToolDispatchService` 兼容路径。
15. 运行态 assistant mode 只能是 `general` / `automation` / `danxi`；旧 `normal` / `auto` / `documents` / `research` / `study` / `office` 只能在边界归一化，不能作为运行态模式保存或暴露。
16. Procedure 已删除。不得恢复 Procedure 表、API、工具、pinned 字段、prompt layer 或 UI；可复用工作流统一使用 SKILL。
17. SKILL 是唯一可复用工作流指导层；`list_skills` / `load_skill` / `create_skill` 是公开入口，SKILL 查询必须匹配标题、摘要、场景和推荐工具，能力暴露必须继续走 CapabilityRegistry / SemanticRouter / ToolRouter / ExecutionTarget。
