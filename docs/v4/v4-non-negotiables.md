...
14. 运行态 assistant mode 只能是 `general` / `automation` / `danxi`；旧 `normal` / `auto` / `documents` / `research` / `study` / `office` 只能在边界归一化，不能作为运行态模式保存或暴露。
15. Procedure 已删除。不得恢复 Procedure 表、API、工具、pinned 字段、prompt layer 或 UI；可复用工作流统一使用 SKILL。
16. SKILL 是唯一可复用工作流指导层；`list_skills` / `load_skill` / `create_skill` 是公开入口，能力暴露必须继续走 CapabilityRegistry / SemanticRouter / ToolRouter / ExecutionTarget。
