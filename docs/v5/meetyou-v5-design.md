...
Research mode is selectable from the desktop composer. The first research UI slice exposes ResearchTask intake, web-search discovery controls, direct webpage URL inputs, academic source adapter/limit controls, PDF/DOCX export controls, plan viewing/editing before start, approve/start/cancel actions, task refresh, pollable runner stage progress, durable RunEvent stream preview, evidence/source preview, summary preview, artifact metadata, and artifact download. While visible tasks are `running`, the desktop UI auto-refreshes ResearchTask state on a short bounded interval and refreshes project artifacts when the active run leaves `running`, so ordinary progress tracking does not depend on manual refresh. Runtime `start` binds a ResearchTask to a Core Run and writes durable `research.*` RunEvents; clients can read `/runtime/research-tasks/{id}/events` with `after_seq` for resumable progress, while compact progress remains available in task metadata. `start` now reaches a conservative Core read-only runner that can gather implemented academic adapters, direct web seed URLs, governed web-search discovery, and project sources into an evidence ledger, persist a Markdown report artifact, and create requested PDF/DOCX derived artifacts. Evidence ledger entries are marked as untrusted evidence-only source text so embedded source instructions are ignored during synthesis. Runtime cancellation is cooperative: once Core observes `cancelled`, the runner stops at the next stage/source boundary and does not create report artifacts or thread delivery messages. Thread-bound completed tasks also deliver a persisted assistant Message with a short summary and artifact link(s); the message is attached to the active branch for checkpoint/version visibility, and desktop Markdown rendering routes those artifact links through the authenticated artifact download proxy. Full deep-research parity still requires richer ranking/reading and broader provider controls.

The main Electron window is a constrained floating surface (`400x620` by default, `340x460` minimum). Project/Thread UI must be validated at the real main-window size, not only in wide browser viewports. At the default width the V5 Project / Thread / Branch / Source / Artifact dock is integrated into the titlebar row. Compact controls may collapse to icons and the thread selector may shrink, but the dock must remain visible, non-overlapping, clickable, and must not wrap into another row or steal chat vertical space. Existing titlebar tools remain part of the top row; when width is insufficient they collapse into a Chinese compact tools menu instead of disappearing.
