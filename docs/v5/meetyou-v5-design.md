...
Research mode is selectable from the desktop composer. The first research UI slice exposes ResearchTask intake, web-search discovery controls, direct webpage URL inputs, academic source adapter/limit controls, plan viewing/editing before start, approve/start/cancel actions, task refresh, pollable runner stage progress, evidence/source preview, summary preview, artifact metadata, and artifact download. `start` now reaches a conservative Core read-only runner that can gather implemented academic adapters, direct web seed URLs, governed web-search discovery, and project sources into an evidence ledger and persist a Markdown report artifact. Runtime cancellation is cooperative: once Core observes `cancelled`, the runner stops at the next stage/source boundary and does not create report artifacts or thread delivery messages. Thread-bound completed tasks also deliver a persisted assistant Message with a short summary and artifact link; the message is attached to the active branch for checkpoint/version visibility, and desktop Markdown rendering routes those artifact links through the authenticated artifact download proxy. Full deep-research parity still requires later streaming progress, richer ranking/reading, broader provider controls, and PDF/DOCX derivation.
