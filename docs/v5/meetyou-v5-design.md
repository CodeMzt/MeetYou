...
Research mode is selectable from the desktop composer. The first research UI slice exposes ResearchTask intake, plan viewing/editing before start, approve/start/cancel actions, task refresh, evidence/source preview, summary preview, artifact metadata, and artifact download. `start` now reaches a conservative Core read-only runner that can gather implemented academic adapters, direct web seed URLs, and project sources into an evidence ledger and persist a Markdown report artifact. Full deep-research parity still requires later progress streaming, richer ranking/reading, tool-backed web-search discovery, and PDF/DOCX derivation.
